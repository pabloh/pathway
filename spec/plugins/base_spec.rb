...
RSpec.describe Pathway::Plugins::Base do
  before do
    stub_const("OperationWithSteps", Class.new(Pathway::Operation) do
  ...
  before do
    allow(validator).to receive(:call) do |state|
      input = state[:input]
      input.key?(:foo) ? input : Pathway::Result.failure(:validation)
    end
  ...
  describe ".process" do
    it "defines a 'call' method wich saves operation argument into the :input key" do
      expect(validator).to receive(:call) do |state|
        ...
        Pathway::Result.failure(:validation)
      end
  ...
  describe "#around" do
    ...
    it "is skipped altogether on a failure state" do
      allow(back_end).to receive(:call).and_return(Pathway::Result.failure(:not_available))
    ...
    context "when running callbacks after the operation has failled" do
      ...
      let(:operation_class) do
        Class.new(Pathway::Operation) do
          context :logger
